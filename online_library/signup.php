...
// Après la soummission du formulaire de compte (plus bas dans ce fichier)
if (TRUE === isset($_POST['signup'])) {
// On vérifie si le code captcha est correct en comparant ce que l'utilisateur a saisi dans le formulaire $_POST["vercode"] et la valeur initialisée $_SESSION["vercode"] lors de l'appel à captcha.php (voir plus bas)
    if ($_POST['vercode'] != $_SESSION['vercode']) {
        ...
        if ($lastId) {
            echo "<script> alert('Votre compte a bien été créé. Votre identifiant lecteur est '+'" . $readerId[0] . "')</script>";
        }
        else {
